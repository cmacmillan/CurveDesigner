        ...
        public static MeshGeneratorOutput GenerateMesh(MeshGeneratorData data)
        {
            ...
            */
            ...
            }
            {
                ...
                {
                    var curr = submeshInfo[i];
                    int minVertIndex = int.MaxValue;
                    int maxVertIndex = int.MinValue;
                    for (int j = curr.indexStart; j < curr.indexStart + curr.indexCount; j++)
                    {
                        int index = triangles[j];
                        Vector3 pos = vertexItems[index].position;
                        ...
                        minVertIndex = Mathf.Min(minVertIndex,index);
                        maxVertIndex = Mathf.Max(maxVertIndex, index);
                    }
                    curr.firstVertex = minVertIndex;
                    curr.vertexCount = maxVertIndex - minVertIndex + 1;
                }
