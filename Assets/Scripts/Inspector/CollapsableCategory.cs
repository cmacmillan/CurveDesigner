...
namespace Assets.NewUI
{
    ...
    public class MainCollapsableCategory : CollapsableCategory
    {
        ...
        internal static void DragNumberValue(Rect dragHotZone, int id, bool isDouble, ref double doubleVal, ref long longVal, double dragSensitivity)
        {
            ...
            switch (evt.GetTypeForControl(id))
            {
                case EventType.MouseDown:
                    if (HitTest(dragHotZone, evt) && evt.button == 0)
                    {
                        ...
                        switch (s_DragCandidateState)
                        {
                            ...
                            case 2:
                                ...
                                if (isDouble)
                                {
                                    ...
                                    doubleVal = RoundBasedOnMinimumDifference(doubleVal, s_DragSensitivity);
                                }
                                else
                                {
                                    longVal += (long)System.Math.Round(HandleUtility.niceMouseDelta * s_DragSensitivity);
                                }
                                GUI.changed = true;
        ...
        {
            return HitTest(rect, evt.mousePosition, false);//evt.isDirectManipulationDevicde);
        }
